        ...
        payment: paymentInfo
            ? {
                  endDate: paymentInfo.endDate
                      ? dayjs(paymentInfo.endDate).format(
                            DATE_TIME_FORMAT.YYYY_MM_DD_HYPHEN,
                        )
                      : undefined,
                  startDate: paymentInfo.startDate
                      ? dayjs(paymentInfo.startDate).format(
                            DATE_TIME_FORMAT.YYYY_MM_DD_HYPHEN,
                        )
                      : undefined,
              }
...
export function convertToPaymentHistory(
    ...
    return {
        ...
        paymentDate: paymentDate
            ...
            : '',
        editDate: editDate
            ? dayjs(editDate).format(DATE_TIME_FORMAT.YYYY_MM_DD_HYPHEN)
            : '',
    };
}
...
export function removeEmptyPromotions(promotions?: IPromotionInfo[]) {
    return promotions?.filter((item) => !isEmpty(item));
}
